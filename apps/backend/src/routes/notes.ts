...
notesRouter.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const blocksOut = await ddb.send(
        new QueryCommand({
          TableName: TABLE_NAME,
          KeyConditionExpression: "PK = :pk AND begins_with(SK, :prefix)",
          ExpressionAttributeValues: {
      ...
      const blocks = (blocksOut.Items ?? []).sort(
    ...
    try {
      ...
      if (Array.isArray(blocks) && blocks.length > 0) {
        ...
        if (content !== undefined) {
          updateExpr += ", content = :content";
          exprValues[":content"] = content;
        }
        ...
        if (folderId !== undefined) {
          updateExpr += ", folderId = :folderId";
          exprValues[":folderId"] = folderId;
        }
        if (isPinned !== undefined) {
          updateExpr += ", isPinned = :isPinned";
          exprValues[":isPinned"] = isPinned;
        }
        if (isArchived !== undefined) {
          updateExpr += ", isArchived = :isArchived";
          exprValues[":isArchived"] = isArchived;
        }
        if (isTrashed !== undefined) {
          updateExpr += ", isTrashed = :isTrashed";
          exprValues[":isTrashed"] = isTrashed;
        }
      ...
      const updatedOut = await ddb.send(
        new GetCommand({
          TableName: TABLE_NAME,
          Key: { PK: userPK(req.userId), SK: noteSK(noteId) },
        }),
